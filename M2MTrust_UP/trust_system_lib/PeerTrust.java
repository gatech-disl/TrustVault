 ...
 */
public class PeerTrust implements TrustAlg{
...
//	double [] current_precision;
	...
	private double[][] PeerTrust_RLT_normalized; 
...
//	private double [][] ini_trust;
	...
	//	LT_vectorB =new double [nw.GLOBALS.NUM_USERS][nw.GLOBALS.NUM_USERS];
		...
		PeerTrust_RLT_normalized=new double[nw.GLOBALS.NUM_USERS][nw.GLOBALS.NUM_USERS];
...
//		current_precision =new double [nw.GLOBALS.NUM_USERS];
		...
		for(int i=0;i<nw.GLOBALS.NUM_USERS;i++){
			for(int j=0;j<nw.GLOBALS.NUM_USERS;j++){
				...
				LT_normalized[i][j]=0;
				PeerTrust_RLT_normalized[i][j]=0;
	...
	public void computeTrust(int user, int cycle){
		//setLinkedlist();
		calsimilarity(cycle);
		active_computetrust(user, 0, cycle);
	}
	private void active_computetrust(int user, int iter, int cycle){
		  setweight_linearthreshold(cycle);	  
		  trustMultiply_uniformPropagation( iter, cycle);  // compute GroupTrust with uniform propagation

		  /*
		//----PeerTrust with controlled propagation (PeerTrust_RLT & PeerTrust)-----------
		  ...
		  if(cycle==nw.GLOBALS.NUM_TRANS+nw.GLOBALS.WARMUP-1){
		     shownormalized();
			  ...
			  if(cycle==nw.GLOBALS.NUM_TRANS+nw.GLOBALS.WARMUP-1){
			    ...
			    LT_trustMultiply_PeerTrust( iter, cycle); // for each peer, calculate the others' reputation.			  
		      //---end PeerTrust 
		    
	       //-----------end PeerTrust controlled propagation---------------------
		     */
	...
	private double[] singleMultiply_uniformPropagation(double[] prev_vector){
	  ...
	  protected double[] LT_trustMultiply_PeerTrust_RLT( int max_iters, int cycle){
			int currt_iter=0;

			cal_initialtrust(cycle);
			
			vectorA = singleMultiply_PeerTrust_RLT(inititrust);
			max_iters--;
			currt_iter++;
			//max_iters--;
			if(cycle==nw.GLOBALS.WARMUP+nw.GLOBALS.NUM_TRANS-1){
			...
			do{ // Multiply until convergence or maximum iterations reached
				...
				vectorB = singleMultiply_PeerTrust_RLT(vectorA);
				currt_iter++;
				max_iters--;
				if(cycle==nw.GLOBALS.WARMUP+nw.GLOBALS.NUM_TRANS-1){
					showreputation(currt_iter, vectorB);
				...
				vectorA = singleMultiply_PeerTrust_RLT(vectorB);
				max_iters--;
				currt_iter++;
				if(cycle==nw.GLOBALS.WARMUP+nw.GLOBALS.NUM_TRANS-1){
				    showreputation(currt_iter, vectorA);
	  ...
	  protected double[] LT_trustMultiply_PeerTrust( int max_iters, int cycle){
			int currt_iter=0;

			cal_initialtrust(cycle);
			
			vectorA = singleMultiply_PeerTrust(inititrust);
			max_iters--;
			currt_iter++;
			//max_iters--;
			if(cycle==nw.GLOBALS.WARMUP+nw.GLOBALS.NUM_TRANS-1){
			...
			do{ // Multiply until convergence or maximum iterations reached
				...
				vectorB = singleMultiply_PeerTrust(vectorA);
				currt_iter++;
				max_iters--;
				if(cycle==nw.GLOBALS.WARMUP+nw.GLOBALS.NUM_TRANS-1){
					showreputation(currt_iter, vectorB);
				...
				vectorA = singleMultiply_PeerTrust(vectorB);
				max_iters--;
				currt_iter++;
				if(cycle==nw.GLOBALS.WARMUP+nw.GLOBALS.NUM_TRANS-1){
				    showreputation(currt_iter, vectorA);
...
//			fback_int = 0;
	 ...
	 * @return A vector closer to converged global trust than that passed in
	 */
	private double[] singleMultiply_PeerTrust(double[] prev_vector){
	...
	private double[] singleMultiply_PeerTrust_RLT(double[] prev_vector){
		double[] lhs = vectorMatrixMult(prev_vector, PeerTrust_RLT_normalized);
		lhs = constantVectorMult((1-ALPHA), lhs);
		double[] rhs = constantVectorMult(ALPHA, pretrust);
		return (vectorAdd(lhs,rhs));	
	}
...
//				}
				...
				System.out.println("fraction of no-activation time window after finishing the transactions is: "+ (double)no_act_timewindow/totl_timewindow+",  number of no-activation time windows= "+no_act_timewindow+", total number of time windows= "+totl_timewindow);
		}
...
//	     System.out.println("threshold_value("+i+","+j+")="+threshold_value);
  ...
  private void rebuiltlocaltrustmatrix_PeerTrust_RLT(int cycle){
	  double sum, sumsim;
	  double [] weightsum = new double [nw.GLOBALS.NUM_USERS];
	  double [] Simsum = new double [nw.GLOBALS.NUM_USERS];
	  double [][] newsim =new double [nw.GLOBALS.NUM_USERS][nw.GLOBALS.NUM_USERS];
	  ...
	  for(int i=0;i<nw.GLOBALS.NUM_USERS;i++){
		  if(weightsum[i]!=0){
		    for(int j=0;j<nw.GLOBALS.NUM_USERS;j++){
			  if(currentactivelist[i].contains(j)){
			         PeerTrust_RLT_normalized[i][j]=normalized[i][j]/weightsum[i];
			  } 
			  else{
				  PeerTrust_RLT_normalized[i][j]=0;
			  }
	  ...
	  for(int i=0;i<nw.GLOBALS.NUM_USERS;i++){
		  sum1=0;
		  for(int j=0;j<nw.GLOBALS.NUM_USERS;j++){
			  sum1+=PeerTrust_RLT_normalized[i][j];
		  }
		  normalizedsum1[i]=sum1;
	  }
	  ...
	  for(int i=0;i<nw.GLOBALS.NUM_USERS;i++){
		  ...
		  for(int j=0;j<nw.GLOBALS.NUM_USERS;j++){
			  if(normalizedsum1[i]!=0)
				  PeerTrust_RLT_normalized[i][j]=PeerTrust_RLT_normalized[i][j]/normalizedsum1[i];
			  else
				  PeerTrust_RLT_normalized[i][j]=0;  
		  }
	  }
	  ...
	  if(cycle==nw.GLOBALS.NUM_TRANS+nw.GLOBALS.WARMUP-1){
	     System.out.println();
	     for(int i=0; i<nw.GLOBALS.NUM_USERS;i++){
		    System.out.print("PeerTrust_RLT_normalized["+i+"]= "+normalizedsum1[i]+"  ");
	     }
	  }
  }
...
//		System.out.println("The average out_rating percentage from the viewpoint of entire network is: " +summ/ (double) b);
	...
	private void showPeerTrust_RLT_normalized(){
		System.out.println("\r\n");
		double sum[] =new double[nw.GLOBALS.NUM_USERS];
		int[] out_cunt= new int[nw.GLOBALS.NUM_USERS];
		int[] in_cunt=new int[nw.GLOBALS.NUM_USERS];
		...
		for(int i=0;i<nw.GLOBALS.NUM_USERS;i++){
			 a=0; c=0;
			for(int j=0;j<nw.GLOBALS.NUM_USERS;j++){
				System.out.print(PeerTrust_RLT_normalized[i][j]+" ");
				if(PeerTrust_RLT_normalized[i][j]!=0){
					a++;
				}
				if(PeerTrust_RLT_normalized[j][i]!=0){
					c++;
				}
			}
			System.out.println(" ;");
