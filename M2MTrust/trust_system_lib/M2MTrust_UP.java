 ...
 */
public class M2MTrust_UP implements TrustAlg{
...
//		current_precision =new double [nw.GLOBALS.NUM_USERS];
		...
		System.out.println("M2M_UP");
	}
	
	// ************************** PUBLIC METHODS *****************************

	public void graphOutputForGui(Globals GLOBALS,Network nw) {
		try{
			PrintWriter __pw = new PrintWriter(new File("trustnetwork.js"));
			//print node map
			__pw.println("nn = [");
			
			for(int i=0;i< GLOBALS.NUM_USERS;++i){
				if(nw.getUser(i).getModel()==User.Behavior.USR_PURE || nw.getUser(i).getModel()==User.Behavior.USR_DISG)
					__pw.printf("{data : {id : '%d', name : '%d',faveColor: '#000000', faveShape: 'ellipse'},classes:'nodee'},\n",i,i);
				else if(nw.getUser(i).isPreTrusted())
					__pw.printf("{data : {id : '%d', name : '%d',faveColor: '#1111FF', faveShape: 'triangle'},classes:'nodee'},\n",i,i);
				else __pw.printf("{data : {id : '%d', name : '%d',faveColor: '#11FF11', faveShape: 'rectangle'},classes:'nodee'},\n",i,i);
			}
			__pw.println("];");
			__pw.flush();
			//print edge map
			...
			for(int i=0;i<GLOBALS.NUM_USERS;++i)
				for (int j=0;j<GLOBALS.NUM_USERS;++j){
					if(i!=j)
						if(nw.getUserRelation(i, j).getPos()-nw.getUserRelation(i, j).getNeg()>0)
							__pw.printf("{data : {source : '%d', target: '%d',faveColor: '#CC0000'},classes:'edgee'},\n",i,j);
				}
			__pw.println("];");
			__pw.flush();
			__pw.close();
	...
	public void showtrust(int currt_iter, double[] vectorA){
		graphOutputForGui(nw.GLOBALS, nw);
	}
	...
	private void active_computetrust(int user, int iter, int cycle){
		  ...
		  //----M2MTrust with uniform propagation-----------
		  trustMultiply_uniformPropagation( iter, cycle);  // compute M2MTrust wiht uniform propagation
		  //--------------end M2MTrust with uniform propagation------------
		  
		  /*
		//----M2MTrust with controlled propagation-----------
		  activestatus ( cycle);
		  ...
		  if(cycle==nw.GLOBALS.NUM_TRANS+nw.GLOBALS.WARMUP-1){
		    ...
		    LT_trustMultiply( iter, cycle); // for each peer, calculate the others' reputation.
	  ...
	  protected double[] LT_trustMultiply( int max_iters, int cycle){
			int currt_iter=0;

			cal_initialtrust(cycle);
			
			vectorA = singleMultiply(inititrust);
			max_iters--;
			currt_iter++;
			//max_iters--;
